...
func TestDefferentLensOfKeys(t *testing.T) {
	...
	_, err := GenQueryFromUpdateTx(ctx, GetTestTableMetaInfo(), []types.TxData{txData}, 0)
...
func TestGenUpdateQuery(t *testing.T) {
	...
	query, err := GenQueryFromUpdateTx(ctx, GetTestTableMetaInfo(), []types.TxData{txData1, txData2}, 0)
	require.Nil(t, err)
	assert.Equal(t, "UPSERT INTO path SELECT * FROM AS_TABLE($p0);\n", query.Statement)
	assert.Equal(t, len(query.Params), 1)
	assert.Equal(t, query.Params[0].Name(), "$p0")
	expectedParams := "[" +
...
func TestGenEraseQuery(t *testing.T) {
	...
	query, err := GenQueryFromEraseTx(ctx, GetTestTableMetaInfo(), []types.TxData{txData1, txData2}, 1)
	require.Nil(t, err)
	assert.Equal(t, "DELETE FROM path ON SELECT * FROM AS_TABLE($p1);\n", query.Statement)
	assert.Equal(t, len(query.Params), 1)
	assert.Equal(t, query.Params[0].Name(), "$p1")
	expectedParams := "[" +
		"<|`key1`:15,`key2`:\"15\"|>," +
		"<|`key1`:16,`key2`:\"16\"|>" +
		"]"
...
func TestGenQuery(t *testing.T) {
	...
	query, err := GenQuery(ctx, GetTestTableMetaInfo(), []types.TxData{txData1, txData2, txData3, txData4, txData5})
	require.Nil(t, err)
	expectedResult := "UPSERT INTO path SELECT * FROM AS_TABLE($p0);\n" +
		"DELETE FROM path ON SELECT * FROM AS_TABLE($p1);\n" +
		"UPSERT INTO path SELECT * FROM AS_TABLE($p2);\n"
	...
	expectedParams := "{\"$p0\":" +
		...
		"<|`key1`:16,`key2`:\"16\"|>" +
		"]," +
		"\"$p2\":[<|`key1`:17,`key2`:\"17\",`value1`:\"15\",`value3`:Just(Double(\"1.00000009\"))|>]}"
